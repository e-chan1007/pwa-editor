<template>
  <div :class="{ 'e-form-group': true, 'is-short': short }" :data-type="type">
    <template v-if="type === 'file'">
      ...
      <div class="form">
        <label class="is-input-file"> <slot /><input type="file" :accept="fileAccept" @change="fileChanged"/></label>
      </div>
    </template>
    ...
    <template v-else>
      <label><span v-if="required" class="label-required">必須</span> {{ label }}</label>
      <div class="form">
        <slot />
      </div>
    </template>
